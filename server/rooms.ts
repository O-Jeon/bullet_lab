...
import {
  ...
  type FinishReason,type PublicPuzzle,type QueueStatus,type RandomSize,
...
type Room={
  code:string;mode:RoomMode;stage:Stage;hostId:string;players:Player[];puzzle:PublicPuzzle|null;
...
export type QueueOutcome={status:'queued';queue:QueueStatus}|{
  status:'matched';size:RandomSize;seats:Array<{socketId:string;seat:Seat}>;
};
...
export class RoomManager{
  ...
  private waiting:Record<RandomSize,QueueEntry[]>={2:[],3:[],4:[]};

  snapshot(code:string,now=Date.now()):RoomSnapshot|null{
    const room=this.rooms.get(code);
    if(!room)return null;
    return {
      code:room.code,mode:room.mode,stage:room.stage,round:room.round,
  ...
  private ensureAvailable(socketId:string){
    ...
    if(this.waiting[2].some(e=>e.socketId===socketId)||this.waiting[3].some(e=>e.socketId===socketId)||this.waiting[4].some(e=>e.socketId===socketId))
      throw new RoomError('이미 랜덤 매칭을 기다리고 있어.');
  }
  private makeRoom(name:string,socketId:string,mode:RoomMode,now:number):Seat{
    let code=roomCode();while(this.rooms.has(code))code=roomCode();
    const player=this.newPlayer(name,socketId);
    this.rooms.set(code,{code,mode,stage:'lobby',hostId:player.id,players:[player],
  ...
  create(name:unknown,socketId:string,now=Date.now()):Seat{
    ...
    return this.makeRoom(sanitizeName(name),socketId,'friends',now);
  }
  ...
  /** FIFO matchmaking: equal-size queues are independent, no bots or fake opponents. */
  enqueueRandom(name:unknown,socketId:string,size:unknown,now=Date.now()):QueueOutcome{
    this.ensureAvailable(socketId);
    if(!isRandomSize(size))throw new RoomError('랜덤 대전은 2명, 3명 또는 4명으로 선택해 줘.');
    this.waiting[size].push({name:sanitizeName(name),socketId});
    if(this.waiting[size].length<size)return {status:'queued',queue:{size,waiting:this.waiting[size].length}};
    const group=this.waiting[size].splice(0,size);
    const host=this.makeRoom(group[0].name,group[0].socketId,'random',now);
    ...
    return {status:'matched',size,seats:seats.map(p=>({socketId:p.socketId,
      seat:{room:this.snapshot(host.room.code,now)!,playerId:p.playerId,token:p.token}}))};
  }
  queueSockets(size:RandomSize){return this.waiting[size].map(p=>p.socketId);}
  queueStatus(size:RandomSize):QueueStatus{return {size,waiting:this.waiting[size].length};}
  cancelQueue(socketId:string):RandomSize|null{
    for(const size of [2,3,4] as const){
      const index=this.waiting[size].findIndex(e=>e.socketId===socketId);
      if(index>=0){this.waiting[size].splice(index,1);return size;}
    }
    return null;
  }
  ...
  disconnect(socketId:string,now=Date.now()):string|null{
    const queuedSize=this.cancelQueue(socketId);
    if(queuedSize!==null)return null;
    const seat=this.sockets.get(socketId);if(!seat)return null;
    this.sockets.delete(socketId);
  ...
  submit(socketId:string,moves:unknown,now=Date.now()):{code:string;result:SubmitResult}{
    ...
    // First verified answer wins. The server stops accepting all further submissions immediately.
    player.bestMoves=moves.length;player.submittedAt=now;
    room.winnerId=player.id;room.finishReason='solved';room.stage='finished';
    return {code:room.code,result:{bestMoves:moves.length,improved:true}};
  }
  ...
  tick(now=Date.now()):string[]{
    ...
    for(const room of this.rooms.values()){
      if(room.stage==='playing'&&room.endsAt!==null&&now>=room.endsAt){
        room.stage='finished';room.finishReason='timeout';changed.push(room.code);
      }
      if(room.stage==='lobby'){
