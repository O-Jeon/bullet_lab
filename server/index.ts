...
import type {CreateRequest,Empty,JoinRequest,Result,Seat,SubmitRequest,SubmitResult} from '../src/multiplayer';
...
export function createGameServer(rooms=new RoomManager()){
  ...
  io.on('connection',(socket:Socket)=>{
    const handle=<T>(ack:((r:Result<T>)=>void)|undefined,fn:()=>T) => {
      ...
      try{ack({ok:true,data:fn()});}catch(error){
        ack({ok:false,error:error instanceof RoomError?error.message:'요청을 처리하지 못했어. 잠시 후 다시 시도해 줘.'});
      }
    };
    socket.on('room:create',(payload:CreateRequest,ack:(r:Result<Seat>)=>void)=>handle(ack,()=>{
      const seat=rooms.create(payload?.name,socket.id);
      socket.join(seat.room.code);broadcast(seat.room.code);return seat;
    }));
    socket.on('room:join',(payload:JoinRequest,ack:(r:Result<Seat>)=>void)=>handle(ack,()=>{
      ...
      if(seat.replacedSocketId){
        const old=io.sockets.sockets.get(seat.replacedSocketId);
    ...
    socket.on('room:leave',(_payload:Empty,ack:(r:Result<Empty>)=>void)=>handle(ack,()=>{
      const code=rooms.leave(socket.id);
      if(code){socket.leave(code);broadcast(code);}
      return {};
    }));
    ...
    socket.on('room:submit',(payload:SubmitRequest,ack:(r:Result<SubmitResult>)=>void)=>handle(ack,()=>{
      ...
      if(result.improved)broadcast(code);
      return result;
    }));
    socket.on('room:rematch',(_payload:Empty,ack:(r:Result<Empty>)=>void)=>handle(ack,()=>{
      const code=rooms.rematch(socket.id);broadcast(code);return {};
    }));
    socket.on('disconnect',()=>{const code=rooms.disconnect(socket.id);if(code)broadcast(code);});
  });
...
if(process.env.NODE_ENV!=='test' && process.env.BULLET_LAB_NO_LISTEN!=='1'){
  ...
  createGameServer().httpServer.listen(port,'0.0.0.0',()=>{
    console.log(`[BULLET LAB] server ready on http://localhost:${port}`);
  });
}
