...
import {sortResults,type Empty,type QueueStatus,type RandomSize,type Result,type RoomSnapshot,type Seat,type SubmitResult} from './multiplayer';
...
export default function OnlineGame(){
  ...
  async function randomMatch(){
    ...
    setLoading(true);
    try{window.localStorage.setItem('bullet-lab-nickname',nickname.trim());}catch{/* private mode */}
    const response=await send<QueueStatus>('match:queue',{name:nickname,size:queueSize});
    setLoading(false);
  ...
  return <><main className="page">
    <div className="intro"><div><div className="eyebrow">02 / ONLINE MULTIPLAYER</div><h1>함께 겨뤄보자<span className="title-dot">.</span></h1><p>친구 또는 랜덤 상대와 대전! 최초 정답이 검증되는 순간 게임이 끝나.</p></div>
    ...
    {!room&&<div className="online-entry">
      <section className="entry-card entry-primary"><div className="eyebrow">CREATE OR JOIN</div><h2>친구들과 한 판!</h2><p>회원가입 없이 닉네임만 입력하면 돼. 방을 만들고 QR코드를 보내 줘.</p>
        ...
        <div className="random-match-box">
          ...
          {queued?<div className="queue-status" role="status"><strong>상대를 찾는 중… {queued.waiting}/{queued.size}명</strong><p>같은 인원을 선택한 플레이어가 모이면 자동으로 시작해!</p><button className="quiet-button" onClick={cancelSearch}>매칭 취소</button></div>
      ...
      <section className="entry-card entry-explain"><div className="eyebrow">MATCH RULES</div><h2>대전은 이렇게 진행돼</h2>
        ...
        <div className="rule-item"><span>03</span><div><b>선착순 성공</b><p>첫 정답을 서버가 검증한 순간 모두의 게임이 종료돼.</p></div></div>
    ...
    {room&&room.stage!=='lobby'&&room.puzzle&&<>
      ...
      <div className="workspace">
        <section className="board-panel"><div className="panel-header"><div><span className="panel-index">ROUND {pad(room.round)} / SAME BOARD FOR EVERYONE</span><h2>{room.stage==='finished'?'경기 종료':'선착순 정답 대전'}</h2></div><span className="match-count">{room.players.length}명 플레이 중</span></div>
          ...
          <div className="online-board-container"><GameBoard puzzle={room.puzzle} robots={robots??room.puzzle.robots} selected={selected} onSelect={onSelect} onMove={onMove} disabled={!canPlay}/>
            ...
            {room.stage==='finished'&&<div className="countdown-mask finished-mask"><span>{room.finishReason==='solved'?'FIRST SOLVE!':'TIME UP'}</span><strong>경기 종료</strong><small>최종 결과가 발표됐어.</small></div>}
          </div><BoardLegend/>
        </section>
        <aside className="sidebar"><section className="stat-card"><div className="sidebar-label">LIVE SESSION <span className="live-dot"/></div>
          ...
          <section className="control-card">
            {room.stage==='finished'?<><div className="sidebar-label">FINAL RESULTS</div><h2 className="result-heading">이번 경기 결과</h2><p className="hint-desc">첫 정답 제출자가 1위야. 그 외 플레이어는 미완료로 표시돼.</p></>
  ...
  {room?.stage==='finished'&&<div className="modal-backdrop" role="presentation">
    <section className="result-card match-result-modal" role="dialog" aria-modal="true" aria-labelledby="match-result-title">
      <div className="result-emoji" aria-hidden="true">{room.finishReason==='solved'?'🏆':'⏱'}</div>
      ...
      <p>{winner?`가장 먼저 성공했어! ${winner.bestMoves}번 움직여서 목표에 도달했어.`:'2분 동안 목표에 도달한 플레이어가 없었어.'}</p>
      <div className="final-ranking" aria-label="최종 순위">
        {ordered.map(player=><div className={`final-rank-row ${player.id===room.winnerId?'rank-winner':''}`} key={player.id}>
          <span className="final-rank-label">{player.id===room.winnerId?'1위':'—'}</span>
