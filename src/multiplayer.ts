/** Shared server/client protocol. Ranked games use pre-certified 6+-move puzzles. */
...
export type MatchRule='race'|'fewest';
...
export type RoomSnapshot={
  code:string;mode:RoomMode;rule:MatchRule;stage:Stage;round:number;players:PlayerView[];
...
export type CreateRequest={name:string;rule:MatchRule};
export type JoinRequest={code:string;name:string;token?:string};
export type SubmitRequest={moves:Move[]};
export type QueueRequest={name:string;size:RandomSize;rule:MatchRule};
export type QueueStatus={size:RandomSize;rule:MatchRule;waiting:number};

export const MAX_PLAYERS=8;
export const MATCH_MILLISECONDS=120_000;
export const COUNTDOWN_MILLISECONDS=3_000;

/** Lowest completed move count first; equal counts break by earliest valid submission. */
